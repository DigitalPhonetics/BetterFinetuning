"""
Combine BarlowTwins loss and Triplet loss to optimize the training process
Need to scale the loss value before plus them
"""
import os
import pickle
import time
...
def train(model, device, train_loader, optimizer, epoch, log_interval):
    ...
    for batch_idx, batch in enumerate(tqdm.tqdm(train_loader)):
        ...
        barlowtwins_loss = model.barlowtwins_loss(anchor_out, positive_out)
        triplet_loss = model.triplet_loss(anchor_out, positive_out, negative_out)
        ...
        # print("barlowtwins_loss(scaled): {}/ triplet_loss: {}/ combined_loss:{}".format(barlowtwins_loss, triplet_loss, combined_loss))
    ...
    print(
        "\n BarlowTwins Loss: {}/ Triplet Loss: {}/ Commbined Loss: {}\n".format(
            barlowtwins_loss, triplet_loss, combined_loss
        )
    )
    return (
        np.mean(losses),
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm.tqdm(test_loader)):
            with autocast():
                ...
                barlowtwins_loss = model.barlowtwins_loss(a_out, p_out)
                triplet_loss = model.triplet_loss(a_out, p_out, n_out)
...
def main(
    ...
    model = BarlowTwins_Contrastive(
        output_size=output_size,
        lambd=lambd,
        triplet_margin=margin,
        barlowtwins_lambd=barlowtwins_lambd,
...
def extract_embedding(old_dict_path, file_name, embeddings, new_dict_save_path):
    ...
    for idx, batch in enumerate(embeddings):
        ...
        for idx, (a, log_a, p, log_p, n, log_n) in enumerate(
            ...
            dict_info[log_p].update({"barlow_contrastive_a": a})
            # new_dict[log_a] = {
            #    "contrastive_a": a,
            #    "speaker_id": dict_info[log_a]["speaker_id"]
            # }
            # dict_info[log_p].update({"barlow_contrastive_p": p})
...
if __name__ == "__main__":
    ...
    train_embed_results, test_embed_results = main(
        input_path=NoiseType_cache_path,
        speech_property="Noise",
        rebuild_dataloaders_cache=True,
        batch_size=32,
        output_size=64,
        lambd=0.05,
        num_epoches=100,
    ...
    extract_embedding(
        old_dict_path=NoiseType_cache_path,
        embeddings=train_embed_results,
        file_name="noise_train_new",
        new_dict_save_path=embedding_save_path,
    )
    extract_embedding(
        old_dict_path=NoiseType_cache_path,
        embeddings=test_embed_results,
        file_name="noise_test_new",
        new_dict_save_path=embedding_save_path,
    )
